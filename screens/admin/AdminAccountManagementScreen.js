...
const AdminAccountManagementScreen = ({ onTabPress, onBack }) => {
  const [adminList, setAdminList] = useState(users.filter((user) => user.role === "admin"))
  ...
  const handleEditPermissions = (admin) => {
    Alert.alert("Phân quyền", `Chỉnh sửa quyền cho ${admin.name}`, [
      ...
      { text: "Admin chính", onPress: () => Alert.alert("Cập nhật", "Đã cập nhật quyền Admin chính") },
      { text: "Kiểm duyệt viên", onPress: () => Alert.alert("Cập nhật", "Đã cập nhật quyền Kiểm duyệt viên") },
      { text: "Hỗ trợ", onPress: () => Alert.alert("Cập nhật", "Đã cập nhật quyền Hỗ trợ") },
    ])
  }
  ...
  const handleDeleteAdmin = (adminId, adminName) => {
    Alert.alert("Xóa tài khoản admin", `Bạn có chắc muốn xóa tài khoản ${adminName}?`, [
      { text: "Hủy", style: "cancel" },
      {
        text: "Xóa",
        style: "destructive",
  ...
  const renderAdmin = ({ item }) => (
    <View style={styles.adminAccountCard}>
      ...
      <View style={styles.adminAccountActions}>
        <TouchableOpacity style={styles.permissionButton} onPress={() => handleEditPermissions(item)}>
          <Text style={styles.permissionButtonText}>Phân quyền</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.securityButton} onPress={() => handleToggle2FA(item.id)}>
          <Text style={styles.securityButtonText}>Bảo mật</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.deleteAdminButton} onPress={() => handleDeleteAdmin(item.id, item.name)}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <AdminBottomNav onTabPress={onTabPress} activeTab="adminAccountManagement" />
    </SafeAreaView>
  )
}
