...
export default function App() {
  ...
  const handleWorkerPress = (worker, service) => {
    ...
    setSelectedService(service);
    setSelectedOrder(null); // Xoá order cũ nếu có
  ...
  const handleRebook = (worker, service, order) => {
    setSelectedWorker(worker);
    setSelectedService(service);
    setSelectedOrder(order); // 👈 lưu đơn cũ
    setCurrentScreen('workerDetail');
  };
  ...
  const renderCurrentScreen = () => {
    switch (currentScreen) {
      ...
      case 'workerInfo':
        return (
          <WorkerInfoScreen onTabPress={handleTabPress} onBack={handleBack} />
        );
      case 'workerArea':
        return (
      ...
      case 'workerIncome':
        return (
          <WorkerIncomeScreen onTabPress={handleTabPress} onBack={handleBack} />
        );
      case 'workerReviews':
        return (
          <WorkerReviewsScreen
